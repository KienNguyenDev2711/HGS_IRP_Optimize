...
static int getDepotIdx(Noeud* place) {
    if (place && place->route && place->route->depot)
        return place->route->depot->cour;
    return -1;
}
...
PLFunction::PLFunction(Params *params, vector<Insertion> insertions, int day, int client) : params(params)
{
    ...
    int curDepotIdx = -1, preDepotIdx = -1;

    // loop through all pieces
    while (index != insertions.end())
    {
        curDepotIdx = getDepotIdx(index->place);
        ...
        if (index == insertions.begin())
        {
            ...
            pre_y = index->detour;
            x = index->load;
            y = calculateCost(day, client, index->detour, x, index->load, curDepotIdx);
            
        }
        else
        {
            double current_cost = calculateCost(day, client, index->detour, index->load, index->load, curDepotIdx);
            std::vector<Insertion>::iterator pre_index = index - 1;
            int preIdxDepot = getDepotIdx(pre_index->place);
            double pre_insertion_cost_with_current_demand = calculateCost(day, client, pre_index->detour, index->load, pre_index->load, preIdxDepot);
            ...
            {    // make piecey
                ...
                y = calculateCost(day, client, pre_detour, x, pre_load, preDepotIdx);
...
PLFunction::PLFunction(Params *params, vector<Insertion> insertions, int day, int client,bool st) : params(params)
{
    ...
    while (index != insertions.end())
    {
        ...
        {
            pre_x = 0;
            pre_y = calculateCost_holding(day, client, index->detour, pre_x,  pre_x, curDepotIdx);
            x = index->load;
            y = calculateCost_holding(day, client, index->detour, x, index->load, curDepotIdx);
            
        }
        else
        {
            double current_cost = calculateCost_holding(day, client, index->detour, index->load, index->load, curDepotIdx);
            std::vector<Insertion>::iterator pre_index = index - 1;
            int preIdxDepot = getDepotIdx(pre_index->place);
            double pre_insertion_cost_with_current_demand = calculateCost_holding(day, client, pre_index->detour, index->load, pre_index->load, preIdxDepot);
            ...
            if (!is_dominated_vehicle)
            {
                ...
                y = calculateCost_holding(day, client, pre_detour, x, pre_load, preDepotIdx);
        ...
        {
            // make piecey
            pre_y = calculateCost_holding(day, client, index->detour, pre_x, index->load, curDepotIdx);
            shared_ptr<LinearPiece> tmp(make_shared<LinearPiece>(pre_x, pre_y, x, y));
            tmp->fromInst = make_shared<Insertion>(index->detour, index->load, index->place);
...
PLFunction::PLFunction(Params *params, vector<Insertion> insertion, int day, int client, bool st, int daily) : params(params)
{
    ...
    while (index != insertions.end())
    {
        ...
        if (index == insertions.begin()){
            pre_x = 0;
            pre_y = calculateCost_stockout(day, client, index->detour, pre_x, pre_x, curDepotIdx);
            x = index->load;
            y = calculateCost_stockout(day, client, index->detour, x, index->load, curDepotIdx);
        }
        else{
            double current_cost = calculateCost_stockout(day, client, index->detour, index->load, index->load, curDepotIdx);
            std::vector<Insertion>::iterator pre_index = index - 1;
            int preIdxDepot = getDepotIdx(pre_index->place);
            double pre_insertion_cost_with_current_demand=calculateCost_stockout(day, client, pre_index->detour, index->load, pre_index->load, preIdxDepot);
            bool is_dominated_vehicle = (index->load <= pre_load) || (pre_insertion_cost_with_current_demand <= current_cost);
            ...
            if (!is_dominated_vehicle  )
            {
                ...
                y = calculateCost_stockout(day, client, pre_detour, x, pre_load, preDepotIdx);
        ...
        {
            // make piecey
            pre_y= calculateCost_stockout(day, client, index->detour, pre_x, index->load, curDepotIdx);
            shared_ptr<LinearPiece> tmp(make_shared<LinearPiece>(pre_x, pre_y, x, y));
            tmp->fromInst = make_shared<Insertion>(index->detour, index->load, index->place);
...
double PLFunction::calculateCost(int day, int client, double detour, double demand, double freeload, int depotIdx)
{
    // detour
    double cost = detour;
    
    double depotInvCost = params->inventoryCostSupplier;
    if (depotIdx >= 0 && depotIdx < (int)params->inventoryCostPerDepot.size())
        depotInvCost = params->inventoryCostPerDepot[depotIdx];
    double inventoryCost = (params->cli[client].inventoryCost - depotInvCost) 
...
double PLFunction::calculateDemandFromCost(int day, int client, double detour, double cost, double freeload, int depotIdx)
{
    double depotInvCost = params->inventoryCostSupplier;
    if (depotIdx >= 0 && depotIdx < (int)params->inventoryCostPerDepot.size())
        depotInvCost = params->inventoryCostPerDepot[depotIdx];
    double inventoryCost = (params->cli[client].inventoryCost - depotInvCost) * (double)(params->ancienNbDays - day);
...
double PLFunction::calculateCost_stockout(int day, int client, double detour, double replenishment, double freeload, int depotIdx)
{
    // detour
    double cost = detour;
    //stock-out cost
    ...
    if (depotIdx >= 0 && depotIdx < (int)params->inventoryCostPerDepot.size())
        depotInvCost = params->inventoryCostPerDepot[depotIdx];
    cost -= depotInvCost * replenishment * (double)(params->ancienNbDays - day); 
...
double PLFunction:: calculateCost_holding(int day, int client, double detour, double replenishment, double freeload, int depotIdx)
{
    // detour
    double cost = detour;
    // holding cost
