...
void Genetic::gererPenalites()
{
	...
	if (fractionTW < params->minValides && params->penalityTimeWindow < 10000)
...
int Genetic::crossPOX2()
{
	...
	if (params->hasTimeWindows)
	{
		for (int k = 1; k <= params->nbDays; k++)
		{
			std::sort(rejeton->chromT[k].begin(), rejeton->chromT[k].end(),
				[&](int a, int b) { return params->cli[a].earliestTime < params->cli[b].earliestTime; });
		}
	}
