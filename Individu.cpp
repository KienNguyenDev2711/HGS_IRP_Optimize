...
int Individu::splitSimple(int k)
{
	...
	double load, distance, time, time2, cost, twViol, arrTime;
	int s0, s1, sb, j;
	potentiels[1][0] = 0;
	...
	for (int i = 0; i < (int)chromT[k].size(); i++)
	{
		...
		while (j < (int)chromT[k].size() && load <= params->ordreVehicules[k][0].vehicleCapacity * params->borneSplit)
		{
			...
			// Time window violation check during split
			if (params->hasTimeWindows)
			{
				arrTime = time;
				if (arrTime < params->cli[s1].earliestTime)
					time = params->cli[s1].earliestTime; // wait
				if (arrTime > params->cli[s1].latestTime)
					twViol += arrTime - params->cli[s1].latestTime;
			}
...
void Individu::measureSol()
{
	...
	coutSol.capacityViol = 0;
	coutSol.lengthViol = 0;
	coutSol.timeWindowViol = 0;
	coutSol.inventoryViol = 0;
	int nbServices = 0;

	// Track which depot serves each customer on each day (for per-depot inventory cost)
	vector<vector<int>> depotOfCustomer(params->nbDays + 1, vector<int>(params->nbDepots + params->nbClients, 0));
	...
	for (int kk = 1; kk <= params->nbDays; kk++)
	{
		...
		for (int jj = 0; jj < params->nombreVehicules[kk]; jj++)
		{
			...
			else if (j == i + 1)
			{
				...
				// TW check for single-delivery route
				{
					double arrTime = time;
					if (arrTime < params->cli[chromT[kk][i]].earliestTime)
						time = params->cli[chromT[kk][i]].earliestTime;
					if (arrTime > params->cli[chromT[kk][i]].latestTime)
						coutSol.timeWindowViol += arrTime - params->cli[chromT[kk][i]].latestTime;
				}
			...
			else
			{
				...
				for (int k = i; k <= j - 2; k++)
				{
					...
					{
						double arrTime = time;
						if (arrTime < params->cli[chromT[kk][k]].earliestTime)
							time = params->cli[chromT[kk][k]].earliestTime;
						if (arrTime > params->cli[chromT[kk][k]].latestTime)
							coutSol.timeWindowViol += arrTime - params->cli[chromT[kk][k]].latestTime;
					}
				...
				// infos sommet fin
				// TW check for last customer
				{
					double arrTime = time;
					if (arrTime < params->cli[chromT[kk][j - 1]].earliestTime)
						time = params->cli[chromT[kk][j - 1]].earliestTime;
					if (arrTime > params->cli[chromT[kk][j - 1]].latestTime)
						coutSol.timeWindowViol += arrTime - params->cli[chromT[kk][j - 1]].latestTime;
				}
			...
			// Record depot assignment for each customer in this route
			for (int ci = i; ci < j; ci++)
				depotOfCustomer[kk][chromT[kk][ci]] = depot;
	...
	if (params->isstockout)
	{
		...
		for (int k = 1; k <= params->nbDays; k++)
		{
			for (int cus = params->nbDepots; cus < params->nbDepots + params->nbClients; cus++)
			{
				...
				// Use per-depot inventory cost if available, otherwise fall back to average
				double depotInvCost = (params->nbDepots > 1 && depotOfCustomer[k][cus] < (int)params->inventoryCostPerDepot.size())
					? params->inventoryCostPerDepot[depotOfCustomer[k][cus]]
					: params->inventoryCostSupplier;
				coutSol.fitness -= chromL[k][cus] * (params->ancienNbDays + 1 - k) * depotInvCost;
		...
		for (int k = 1; k <= params->ancienNbDays; k++)
		{
			for (int i = params->nbDepots; i < params->nbDepots + params->nbClients; i++)
			{
				double depotInvCost = (params->nbDepots > 1 && depotOfCustomer[k][i] < (int)params->inventoryCostPerDepot.size())
					? params->inventoryCostPerDepot[depotOfCustomer[k][i]]
					: params->inventoryCostSupplier;
				coutSol.fitness += chromL[k][i] * (params->ancienNbDays + 1 - k) * (params->cli[i].inventoryCost - depotInvCost);
			}
		}
	...
	// Compute inventory violation (for non-stockout mode, informational)
	if (!params->isstockout)
	{
		for (int i = params->nbDepots; i < params->nbDepots + params->nbClients; i++)
		{
			double inv = params->cli[i].startingInventory;
			for (int k = 1; k <= params->nbDays; k++)
			{
				inv += chromL[k][i];
				inv -= params->cli[i].dailyDemand[k];
				if (inv < params->cli[i].minInventory)
					coutSol.inventoryViol += params->cli[i].minInventory - inv;
				if (inv > params->cli[i].maxInventory)
					coutSol.inventoryViol += inv - params->cli[i].maxInventory;
			}
		}
	}
