...
void Route::updateRouteData ()
{
	...
	while ( !noeud->estUnDepot || firstIt )
	{
		...
		if (params->hasTimeWindows && !noeud->estUnDepot)
		{
			double arrivalTime = earlT;
			if (arrivalTime < params->cli[noeud->cour].earliestTime)
				earlT = params->cli[noeud->cour].earliestTime; // wait until TW opens
			if (arrivalTime > params->cli[noeud->cour].latestTime)
				twViol += arrivalTime - params->cli[noeud->cour].latestTime;
		}
	}
	...
	if (charge < vehicleCapacity + 0.0001 && earlT < maxRouteTime + 0.0001 && twViol < 0.0001) isFeasible = true ;
...
void Route::evalInsertClient (Noeud * U) 
{
	...
	if ( U->route != this || !U->estPresent )
	{
		...
		while (!courNoeud->estUnDepot || firstIt == true )
		{
			...
			// TW-aware insertion penalty
			if (params->hasTimeWindows)
			{
				double serviceStartCour = courNoeud->estUnDepot ? 0.0 
					: std::max(courNoeud->est, params->cli[courNoeud->cour].earliestTime);
				double arrivalU = serviceStartCour + params->cli[courNoeud->cour].serviceDuration 
					+ params->timeCost[courNoeud->cour][U->cour];
				if (arrivalU > params->cli[U->cour].latestTime)
					cout += (arrivalU - params->cli[U->cour].latestTime) * params->penalityTimeWindow;
			}
